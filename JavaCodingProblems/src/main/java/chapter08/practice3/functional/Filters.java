...
public final class Filters {
    ...
    public static List<Melon> filterMelon(List<Melon> melons, MelonPredicate mp) {
        if (melons == null) throw new IllegalArgumentException("parameter can not be null");

        List<Melon> result = new ArrayList<>();
        for (Melon m : melons) {
            if (m != null && mp.test(m)) result.add(m);
        }
        return result;
    }
